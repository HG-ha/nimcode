...
pub struct OpenAiCompatConfig {
    ...
    /// Maximum request body size in bytes. NVIDIA NIM accepts large multimodal
    /// payloads but the integrate.api.nvidia.com chat endpoint historically caps
    /// at ~10MB. We pick a conservative 10MB to surface clear errors before the
    /// upstream returns an opaque 413.
    pub max_request_body_bytes: usize,
}

const NIM_ENV_VARS: &[&str] = &["NVIDIA_NIM_API_KEY"];

const NIM_MAX_REQUEST_BODY_BYTES: usize = 10_485_760; // 10MB

impl OpenAiCompatConfig {
    /// NVIDIA NIM (`integrate.api.nvidia.com/v1`) — OpenAI-compatible chat
    /// completions endpoint. Free tier is 40 req/min and supports models like
    /// `z-ai/glm-5.1`, `moonshotai/kimi-k2.5`, `minimaxai/minimax-m2.5`, etc.
    #[must_use]
    pub const fn nim() -> Self {
        Self {
            ...
            max_request_body_bytes: NIM_MAX_REQUEST_BODY_BYTES,
        }
    }
    ...
    /// Backwards-compatible aliases for the deleted xAI / OpenAI / DashScope
    /// constructors. After the NIM-only refactor every provider configuration
    /// resolves to `OpenAiCompatConfig::nim()`. The aliases exist so that the
    /// in-tree unit tests (which exercise generic OpenAI-compatible wire
    /// shape) keep linking without each test having to be rewritten.
    #[doc(hidden)]
    #[must_use]
...
impl OpenAiCompatClient {
    ...
    /// Fetch the list of model ids exposed by the configured OpenAI-compatible
    /// endpoint. For NIM this hits `<base>/models` and returns the contents of
    /// the `data[].id` field, sorted ascendingly. The response is intentionally
    /// kept minimal — we expose ids only — so callers can plug it directly
    /// into `/model` autocompletion without taking a dependency on the full
    /// model schema.
    pub async fn list_models(&self) -> Result<Vec<String>, ApiError> {
        let url = models_endpoint(&self.base_url);
        let response = self
            .http
            .get(&url)
        ...
        let parsed: serde_json::Value =
            serde_json::from_str(&body).map_err(|err| ApiError::Json {
...
struct StreamState {
    ...
    /// Buffer for partial `<think>` / `</think>` tag detection so a tag split
    /// across chunk boundaries (e.g. "...<thi" + "nk>...") still parses cleanly.
...
impl StreamState {
    ...
    fn ingest_chunk(&mut self, chunk: ChatCompletionChunk) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        for choice in chunk.choices {
            ...
            if let Some(content) = choice.delta.content.filter(|value| !value.is_empty()) {
                self.process_content_chunk(&content, &mut events);
            }

            for tool_call in choice.delta.tool_calls {
                let openai_index = tool_call.index;
                let state = self.tool_calls.entry(openai_index).or_default();
                state.apply(tool_call);
                if state.block_index.is_none() && state.name.is_some() {
                    let assigned = self.next_block_index;
                    self.next_block_index += 1;
                    state.block_index = Some(assigned);
                }
                let Some(block_index) = state.block_index else {
                    continue;
                };
                if !state.started {
            ...
            if let Some(finish_reason) = choice.finish_reason {
                ...
                if finish_reason == "tool_calls" {
                    for state in self.tool_calls.values_mut() {
                        if let Some(block_index) = state.block_index {
                            if state.started && !state.stopped {
                                state.stopped = true;
                                events.push(StreamEvent::ContentBlockStop(
                                    ContentBlockStopEvent { index: block_index },
                                ));
                            }
                        }
                    }
    ...
    /// Ensure a Thinking content block has been started, closing any open text
    /// block first so Anthropic's "thinking comes before text in content order"
    /// invariant holds.
    fn ensure_thinking_started(&mut self, events: &mut Vec<StreamEvent>) -> u32 {
    ...
    fn ensure_text_started(&mut self, events: &mut Vec<StreamEvent>) -> u32 {
    ...
    fn emit_thinking_delta(&mut self, text: &str, events: &mut Vec<StreamEvent>) {
        let index = self.ensure_thinking_started(events);
    ...
    fn emit_text_delta(&mut self, text: &str, events: &mut Vec<StreamEvent>) {
        let index = self.ensure_text_started(events);
        events.push(StreamEvent::ContentBlockDelta(ContentBlockDeltaEvent {
            index,
    ...
    /// Stream-aware splitter for inline `<think>...</think>` tags. Buffers
    /// partial-tag prefixes across chunks so a tag split mid-token still gets
    /// recognized. Falls through to plain-text emission when thinking is
    /// disabled or no tags are present.
    fn process_content_chunk(&mut self, content: &str, events: &mut Vec<StreamEvent>) {
        if !self.enable_thinking {
            self.emit_text_delta(content, events);
            return;
        }
        self.think_tag_buffer.push_str(content);
        loop {
            if self.inside_think_tag {
                if let Some(pos) = self.think_tag_buffer.find("</think>") {
                    let thinking = self.think_tag_buffer[..pos].to_string();
                    if !thinking.is_empty() {
                        self.emit_thinking_delta(&thinking, events);
                    }
                    self.think_tag_buffer = self.think_tag_buffer[pos + "</think>".len()..]
                        .to_string();
                    self.inside_think_tag = false;
                    continue;
                }
                // No close tag yet. Hold back any trailing prefix that could
                // be the start of "</think>" so we don't emit a partial tag
                // as thinking text.
                let safe_prefix_len = safe_emit_len(&self.think_tag_buffer, "</think>");
                if safe_prefix_len > 0 {
                    let chunk = self.think_tag_buffer[..safe_prefix_len].to_string();
                    self.emit_thinking_delta(&chunk, events);
                    self.think_tag_buffer = self.think_tag_buffer[safe_prefix_len..].to_string();
                }
                return;
            }
            if let Some(pos) = self.think_tag_buffer.find("<think>") {
                let preceding = self.think_tag_buffer[..pos].to_string();
                if !preceding.is_empty() {
                    self.emit_text_delta(&preceding, events);
                }
                self.think_tag_buffer = self.think_tag_buffer[pos + "<think>".len()..].to_string();
                self.inside_think_tag = true;
                continue;
            }
            // Hold back any trailing prefix that could become "<think>".
            let safe_prefix_len = safe_emit_len(&self.think_tag_buffer, "<think>");
            if safe_prefix_len > 0 {
                let chunk = self.think_tag_buffer[..safe_prefix_len].to_string();
                self.emit_text_delta(&chunk, events);
                self.think_tag_buffer = self.think_tag_buffer[safe_prefix_len..].to_string();
            }
            return;
        }
    ...
    fn finish(&mut self) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        let mut events = Vec::new();
        // Flush any leftover content held back from the tag-split buffer. If
        // we ended mid-`<think>` without a close, treat the buffer as text so
        // we don't drop visible output. If we ended mid-thinking, surface it
        // as thinking content rather than dropping it.
        if !self.think_tag_buffer.is_empty() {
            let pending = std::mem::take(&mut self.think_tag_buffer);
            if self.inside_think_tag {
                self.emit_thinking_delta(&pending, &mut events);
            } else {
                self.emit_text_delta(&pending, &mut events);
            }
        }
        if self.thinking_started && !self.thinking_finished {
            self.thinking_finished = true;
        ...
        // Drain tool-calls that arrived but never got their start event
        // emitted (e.g. when the close marker shows up before the function
        // name resolves). We assign block indices here since the chunk loop
        // could not.
        let openai_indices: Vec<u32> = self.tool_calls.keys().copied().collect();
        for openai_index in openai_indices {
            let block_index = {
...
/// Returns how many bytes from the start of `buffer` are safe to emit without
/// hiding a partial occurrence of `tag`. Specifically, we keep the longest
/// trailing slice of `buffer` that could still grow into `tag` once more
/// content arrives. Mirrors the chunked-tag-split logic in the upstream
/// proxy-core SSE module.
fn safe_emit_len(buffer: &str, tag: &str) -> usize {
    let bytes = buffer.as_bytes();
    let max_check = bytes.len().min(tag.len() - 1);
    for hold in (1..=max_check).rev() {
        let suffix_start = bytes.len() - hold;
        // Only consider char-boundary cuts so we never split a UTF-8 sequence.
...
struct ToolCallState {
    ...
    started: bool,
    stopped: bool,
    /// Anthropic block index this tool call has been assigned. `None` until
    /// the first chunk that gives us a `name` so we can emit the start event.
...
impl ToolCallState {
    ...
    fn start_event(&self, block_index: u32) -> Result<Option<ContentBlockStartEvent>, ApiError> {
...
struct ChatMessage {
    ...
    /// Native reasoning channel emitted by NVIDIA NIM thinking models
    /// (GLM, Kimi, QwQ, DeepSeek-R1, etc.) when the upstream surfaces the
    /// chain-of-thought separately from the user-visible content.
    #[serde(default)]
...
struct ChunkDelta {
    ...
    content: Option<String>,
    /// Native reasoning channel emitted by NVIDIA NIM thinking models
    /// (GLM, Kimi, QwQ, DeepSeek-R1, etc.) when the upstream streams the
    /// chain-of-thought separately from user-visible content.
    #[serde(default)]
    reasoning_content: Option<String>,
...
fn should_request_stream_usage(config: OpenAiCompatConfig) -> bool {
    matches!(config.provider_name, "OpenAI" | "NVIDIA NIM")
}

/// Whether the global `ENABLE_THINKING` switch is on. Mirrors the toggle used
/// by the upstream NIM proxy. When enabled (default), reasoning_content
/// fields and `<think>` tags emitted by NIM models (GLM, Kimi, QwQ, etc.) are
/// surfaced as native Anthropic thinking blocks.
fn thinking_enabled() -> bool {
    match std::env::var("ENABLE_THINKING") {
        Ok(value) => !matches!(
...
fn normalize_response(
    ...
    let thinking_on = thinking_enabled();
    ...
    if let Some(text) = choice.message.content.filter(|value| !value.is_empty()) {
        if thinking_on {
            // Split inline `<think>...</think>` so the visible answer never
            // carries the upstream chain-of-thought verbatim.
            extend_with_split_thinking(&text, &mut content);
        } else {
...
/// Non-streaming variant of `<think>...</think>` splitting. Walks `text` once,
/// pushing thinking and text content blocks in encounter order. Multiple think
/// segments are supported. Unbalanced/orphan tags fall through as plain text
/// so we never silently drop content.
fn extend_with_split_thinking(text: &str, content: &mut Vec<OutputContentBlock>) {
    let mut remaining = text;
    while let Some(start) = remaining.find("<think>") {
        let preceding = &remaining[..start];
        if !preceding.is_empty() {
        ...
        let Some(end_rel) = after_open.find("</think>") else {
            // Unbalanced — treat the whole rest of the buffer (including the
            // open tag) as plain text so we don't lose visible characters.
            content.push(OutputContentBlock::Text {
                text: remaining.to_string(),
            });
            return;
        };
        let thinking = after_open[..end_rel].trim();
        if !thinking.is_empty() {
            content.push(OutputContentBlock::Thinking {
        ...
        remaining = &after_open[end_rel + "</think>".len()..];
    }
...
fn models_endpoint(base_url: &str) -> String {
    ...
    if trimmed.ends_with("/models") {
        trimmed.to_string()
    } else if let Some(stripped) = trimmed.strip_suffix("/chat/completions") {
...
mod tests {
    ...
    #[test]
    fn nim_streaming_requests_opt_in_to_usage_in_stream_options() {
        // After the NIM-only refactor every provider config maps to
        // `OpenAiCompatConfig::nim()`. NIM accepts (and we want) the
        // `stream_options.include_usage` opt-in so token counts arrive in
        // the final SSE chunk.
        let payload = build_chat_completion_request(
            &MessageRequest {
    ...
    #[test]
    fn missing_nvidia_nim_api_key_is_provider_specific() {
        let _lock = env_lock();
        std::env::remove_var("NVIDIA_NIM_API_KEY");
        let error = OpenAiCompatClient::from_env(OpenAiCompatConfig::nim())
    ...
    #[test]
    fn check_request_body_size_fails_for_nim_when_exceeds_10mb() {
        // After the NIM-only refactor every config maps to NIM with a
        // conservative 10MB cap on `integrate.api.nvidia.com/v1`. Pump up
        // a payload past that threshold and verify we surface a clear
        // local error before sending the request upstream.
        let large_content = "x".repeat(11_000_000); // 11MB of content
        let request = MessageRequest {
            model: "z-ai/glm-5.1".to_string(),
            max_tokens: 100,
        ...
        let result = super::check_request_body_size(&request, OpenAiCompatConfig::nim());
        assert!(result.is_err(), "should fail for 11MB request to NIM");
        ...
        match err {
            crate::error::ApiError::RequestBodySizeExceeded {
                ...
                assert_eq!(max_bytes, 10_485_760); // 10MB limit
    ...
    #[test]
    fn check_request_body_size_allows_small_requests_for_nim() {
        let small_content = "x".repeat(5_000_000); // 5MB of content
        let request = MessageRequest {
            model: "z-ai/glm-5.1".to_string(),
            max_tokens: 100,
            messages: vec![InputMessage::user_text(small_content)],
            stream: false,
            ..Default::default()
        };

        assert!(
            super::check_request_body_size(&request, OpenAiCompatConfig::nim()).is_ok(),
            "5MB request should pass under NIM's 10MB limit"
        );
    }

    #[test]
    fn nim_size_limit_is_ten_megabytes() {
        // Sanity check the documented per-provider cap. After the NIM-only
        // refactor xai()/openai()/dashscope() are all aliases for nim(), so
        // every alias should report the same 10MB ceiling.
        assert_eq!(OpenAiCompatConfig::nim().max_request_body_bytes, 10_485_760);
        assert_eq!(OpenAiCompatConfig::xai().max_request_body_bytes, 10_485_760);
        assert_eq!(
