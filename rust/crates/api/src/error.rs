    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MissingCredentials {
                ...
                write!(
                    f,
                    "missing {provider} credentials. Set up via:\n\
                     \n  1. Run `nimcode` interactively — it will prompt for the key and save it automatically\
                     \n  2. Set the {} environment variable\
                     \n  3. Create a .env file with {}=<value> in the working directory",
                    env_vars.join(" or "),
                    env_vars.first().unwrap_or(&"API_KEY"),
                )?;
