...
// Windows treats environment variable names as case-insensitive, so the
// "uppercase wins over lowercase" precedence test cannot meaningfully run
// there — setting `HTTP_PROXY` and `http_proxy` to different values just
// overwrites the same key. The tests are gated to non-Windows targets.
#[cfg(not(windows))]
#[test]
