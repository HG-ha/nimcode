...
async fn send_message_blocks_oversized_requests_before_the_http_call() {
    ...
    // After the NIM-only refactor we use a model with a known context
    // window (`z-ai/glm-5.1`, 131_072 tokens) so the local preflight has
    // a budget to compare against. Pump the input well past the window
    // and verify the request fails locally before any HTTP traffic.
    let client = OpenAiCompatClient::new("nvapi-test-key", OpenAiCompatConfig::nim())
        .with_base_url(server.base_url());
    let error = client
...
async fn provider_client_dispatches_xai_requests_from_env() {
    let _lock = env_lock();
    // After the NIM-only refactor `grok` aliases through to a NIM-backed
    // glm5.1 model and authenticates via NVIDIA_NIM_API_KEY. We point the
    // client at the local mock server so the OpenAI-compatible wire shape
    // can be observed end-to-end without leaving the test process.
    let _api_key = ScopedEnvVar::set("NVIDIA_NIM_API_KEY", "nvapi-test-key");
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_provider\",\"model\":\"z-ai/glm-5.1\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Through provider client\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":9,\"completion_tokens\":4}}",
        )],
    )
    .await;
    let _base_url = ScopedEnvVar::set("NVIDIA_NIM_BASE_URL", server.base_url());

    let client =
        ProviderClient::from_model("grok").expect("NIM provider client should be constructed");
    assert!(matches!(client, ProviderClient::Nim(_)));
