...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        summary: "Show or switch the interaction language (zh/en/auto)",
        argument_hint: Some("[zh|en|auto]"),
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "permissions",
    ...
    SlashCommandSpec {
        name: "config",
        aliases: &[],
        summary: "Inspect NimCode config files or merged sections",
    ...
    SlashCommandSpec {
        name: "memory",
        aliases: &[],
        summary: "Inspect loaded NimCode instruction memory files",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
        name: "init",
        aliases: &[],
        summary: "Create a starter NIMCODE.md for this repo",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
    ...
    SlashCommandSpec {
        ...
        summary: "Manage NimCode plugins",
        argument_hint: Some(
...
pub fn validate_slash_command_input(
    ...
    Ok(Some(match command {
        ...
        "model" | "models" => SlashCommand::Model {
            model: optional_single_arg(command, &args, "[model]")?,
        },
        "lang" | "language" | "locale" => SlashCommand::Lang {
            mode: optional_single_arg(command, &args, "[zh|en|auto]")?,
        },
...
enum DefinitionSource {
    ProjectNimcode,
    ProjectCodex,
    ProjectNimcodeLegacy,
    UserNimcodeConfigHome,
    UserCodexHome,
    UserNimcode,
    UserCodex,
    UserNimcodeLegacy,
...
impl DefinitionSource {
    fn report_scope(self) -> DefinitionScope {
        match self {
            Self::ProjectNimcode | Self::ProjectCodex | Self::ProjectNimcodeLegacy => {
                DefinitionScope::Project
            }
            Self::UserNimcodeConfigHome | Self::UserCodexHome => DefinitionScope::UserConfigHome,
            Self::UserNimcode | Self::UserCodex | Self::UserNimcodeLegacy => DefinitionScope::UserHome,
        }
    }
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectNimcode,
            ancestor.join(".nimcode").join(leaf),
        );
        push_unique_root(
            &mut roots,
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectNimcodeLegacy,
            ancestor.join(".nimcode").join(leaf),
        );
    }

    if let Ok(nimcode_config_home) = env::var("NIMCODE_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserNimcodeConfigHome,
            PathBuf::from(nimcode_config_home).join(leaf),
        );
    }
    ...
    if let Ok(nimcode_config_dir) = env::var("NIMCODE_CONFIG_DIR") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserNimcodeLegacy,
            PathBuf::from(nimcode_config_dir).join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserNimcode,
            home.join(".nimcode").join(leaf),
        );
        push_unique_root(
            &mut roots,
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectNimcode,
            ancestor.join(".nimcode").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectNimcode,
            ancestor.join(".nimcode").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(nimcode_config_home) = env::var("NIMCODE_CONFIG_HOME") {
        let nimcode_config_home = PathBuf::from(nimcode_config_home);
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserNimcodeConfigHome,
            nimcode_config_home.join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserNimcodeConfigHome,
            nimcode_config_home.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserNimcode,
            home.join(".nimcode").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserNimcodeLegacy,
            home.join(".nimcode").join("skills").join("omc-learned"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(nimcode_config_dir) = env::var("NIMCODE_CONFIG_DIR") {
        let nimcode_config_dir = PathBuf::from(nimcode_config_dir);
        let skills_dir = nimcode_config_dir.join("skills");
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserNimcodeLegacy,
            skills_dir.clone(),
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserNimcodeLegacy,
            nimcode_config_dir.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Ok(nimcode_config_home) = env::var("NIMCODE_CONFIG_HOME") {
        return Ok(PathBuf::from(nimcode_config_home).join("skills"));
    }
    if let Ok(codex_home) = env::var("CODEX_HOME") {
        return Ok(PathBuf::from(codex_home).join("skills"));
    }
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".nimcode").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set NIMCODE_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       nimcode agents".to_string(),
        "  Sources          .nimcode/agents, ~/.nimcode/agents, $NIMCODE_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "nimcode agents [list|help]",
            "sources": [".nimcode/agents", "~/.nimcode/agents", "$NIMCODE_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       nimcode skills [list|install <path>|help|<skill> [args]]".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $NIMCODE_CONFIG_HOME/skills or ~/.nimcode/skills".to_string(),
        "  Sources          .nimcode/skills, .omc/skills, .agents/skills, .codex/skills, .nimcode/skills, ~/.nimcode/skills, ~/.omc/skills, ~/.nimcode/skills/omc-learned, ~/.codex/skills, ~/.nimcode/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "nimcode skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$NIMCODE_CONFIG_HOME/skills or ~/.nimcode/skills",
            "sources": [
                ".nimcode/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       nimcode mcp [list|show <server>|help]".to_string(),
        "  Sources          .nimcode/settings.json, .nimcode/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "nimcode mcp [list|show <server>|help]",
            "sources": [".nimcode/settings.json", ".nimcode/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
fn definition_source_id(source: DefinitionSource) -> &'static str {
    match source {
        DefinitionSource::ProjectNimcode
        | DefinitionSource::ProjectCodex
        | DefinitionSource::ProjectNimcodeLegacy => "project_nimcode",
        DefinitionSource::UserNimcodeConfigHome | DefinitionSource::UserCodexHome => {
            "user_nimcode_config_home"
        }
        DefinitionSource::UserNimcode | DefinitionSource::UserCodex | DefinitionSource::UserNimcodeLegacy => {
            "user_nimcode"
        }
    }
}
...
mod tests {
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage NimCode plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn lists_agents_from_project_and_user_roots() {
        ...
        let user_agents = user_home.join(".nimcode").join("agents");
    ...
    fn renders_agents_reports_as_json() {
        ...
        assert_eq!(report["agents"][2]["shadowed_by"]["id"], "project_nimcode");

        let help = handle_agents_slash_command_json(Some("help"), &workspace).expect("agents help");
        assert_eq!(help["kind"], "agents");
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["direct_cli"], "nimcode agents [list|help]");
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".nimcode").join("commands");
        ...
        let roots = vec![
            ...
            SkillRoot {
                source: DefinitionSource::ProjectNimcodeLegacy,
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".nimcode").join("skills");
        let legacy_commands = workspace.join(".nimcode").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(report["skills"][0]["source"]["id"], "project_nimcode");
        assert_eq!(report["skills"][1]["name"], "deploy");
        assert_eq!(report["skills"][1]["origin"]["id"], "legacy_commands_dir");
        assert_eq!(report["skills"][3]["shadowed_by"]["id"], "project_nimcode");
        ...
        assert_eq!(
            ...
            "nimcode skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       nimcode agents"));
        assert!(agents_help
            .contains("Sources          .nimcode/agents, ~/.nimcode/agents, $NIMCODE_CONFIG_HOME/agents"));
        ...
        assert!(skills_help.contains("Install root     $NIMCODE_CONFIG_HOME/skills or ~/.nimcode/skills"));
        assert!(skills_help.contains(".omc/skills"));
        assert!(skills_help.contains(".agents/skills"));
        assert!(skills_help.contains("~/.nimcode/skills/omc-learned"));
        ...
        assert!(sources
            .iter()
            .any(|value| value == "~/.nimcode/skills/omc-learned"));
    ...
    fn discovers_omc_skills_from_project_and_user_compatibility_roots() {
        ...
        let nimcode_config_dir = temp_dir("skills-omc-nimcode-config");
        ...
        let nimcode_config_skills = nimcode_config_dir.join("skills");
        let nimcode_config_commands = nimcode_config_dir.join("commands");
        let learned_skills = nimcode_config_dir.join("skills").join("omc-learned");
        let original_home = std::env::var_os("HOME");
        let original_nimcode_config_dir = std::env::var_os("NIMCODE_CONFIG_DIR");
        ...
        write_skill(
            &nimcode_config_skills,
            "statusline",
            "NimCode config skill guidance",
        );
        write_legacy_command(
            &nimcode_config_commands,
            "doctor-check",
            "NimCode config command guidance",
        );
        write_skill(&learned_skills, "learned", "Learned skill guidance");
        std::env::set_var("HOME", &user_home);
        std::env::set_var("NIMCODE_CONFIG_DIR", &nimcode_config_dir);
        ...
        assert!(report.contains("statusline · NimCode config skill guidance"));
        assert!(report.contains("doctor-check · NimCode config command guidance · legacy /commands"));
        ...
        restore_env_var("NIMCODE_CONFIG_DIR", original_nimcode_config_dir);
        let _ = fs::remove_dir_all(workspace);
        let _ = fs::remove_dir_all(user_home);
        let _ = fs::remove_dir_all(nimcode_config_dir);
    }
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       nimcode mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".nimcode")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".nimcode").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".nimcode").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".nimcode/settings.json");
    ...
    fn mcp_degrades_gracefully_on_malformed_mcp_config_144() {
        // #144: mirror of #143's partial-success contract for `nimcode mcp`.
        // Previously `mcp` hard-failed on any config parse error, hiding
        // well-formed servers and forcing nimcode to fall back to `doctor`.
        ...
        fs::create_dir_all(workspace.join(".nimcode")).expect("create workspace .nimcode dir");
        fs::create_dir_all(&config_home).expect("create config home");
        // One valid server + one malformed entry missing `command`.
        fs::write(
            workspace.join(".nimcode.json"),
            r#"{
  "mcpServers": {
