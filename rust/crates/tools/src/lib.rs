...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set NimCode settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("nimcode-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("NIMCODE_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("NIMCODE_TODO_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    Ok(cwd.join(".nimcode-todos.json"))
}
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(nimcode_config_home) = std::env::var("NIMCODE_CONFIG_HOME") {
        push_prefixed_skill_lookup_roots(&mut roots, std::path::Path::new(&nimcode_config_home));
    }
    ...
    if let Ok(nimcode_config_dir) = std::env::var("NIMCODE_CONFIG_DIR") {
        let nimcode_config_dir = std::path::PathBuf::from(nimcode_config_dir);
        push_skill_lookup_root(
            &mut roots,
            nimcode_config_dir.join("skills"),
            SkillLookupOrigin::SkillsDir,
        );
        push_skill_lookup_root(
            &mut roots,
            nimcode_config_dir.join("skills").join("omc-learned"),
            SkillLookupOrigin::SkillsDir,
        );
        push_skill_lookup_root(
            &mut roots,
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.nimcode/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".nimcode"));
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".codex"));
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".nimcode-compat"));
    }
}
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".nimcode"));
    push_prefixed_skill_lookup_roots(roots, &home.join(".codex"));
    push_prefixed_skill_lookup_roots(roots, &home.join(".nimcode-compat"));
    push_skill_lookup_root(
        roots,
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("nimcode-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("NIMCODE_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".nimcode-agents"));
    }
    Ok(cwd.join(".nimcode-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "nimcodeguide" | "nimcodeguideagent" | "guide" => String::from("nimcode-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".nimcode").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("NIMCODE_CONFIG_HOME") {
        ...
        .map_err(|_| {
            String::from(
                "HOME is not set (on Windows, set USERPROFILE or HOME, \
                 or use NIMCODE_CONFIG_HOME to point directly at the config directory)",
            )
        })?;
    Ok(PathBuf::from(home).join(".nimcode"))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("nimcode-tools-{unique}-{name}"))
    }
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .nimcode/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let nimcode_dir = worktree.join(".nimcode");
        fs::create_dir_all(&nimcode_dir).expect("create .nimcode dir");
        ...
        fs::write(nimcode_dir.join("settings.json"), settings).expect("write settings");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .nimcode/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".nimcode").join("worker-state.json");
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        // sets NIMCODE_WEB_SEARCH_BASE_URL. Without the lock, parallel test
        ...
        std::env::remove_var("NIMCODE_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("NIMCODE_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("NIMCODE_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("NIMCODE_TODO_STORE");
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".nimcode").join("skills").join("plan");
        let command_dir = root.join(".nimcode").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".nimcode/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".nimcode/commands/handoff.md"));
    ...
    #[test]
    fn skill_loads_project_local_compat_skill_prompt() {
        let _guard = env_guard();
        let root = temp_path("project-skills");
        let home = root.join("home");
        let workspace = root.join("workspace");
        let nested = workspace.join("nested");
        let skill_dir = workspace.join(".nimcode-compat").join("skills").join("trace");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(output["path"]
            .as_str()
            .expect("path")
            .ends_with(".nimcode-compat/skills/trace/SKILL.md"));
    ...
    #[test]
    fn skill_loads_learned_skill_from_nimcode_config_dir() {
        let _guard = env_guard();
        let root = temp_path("nimcode-config-learned-skill");
        let home = root.join("home");
        let nimcode_config_dir = root.join("nimcode-config");
        let learned_skill_dir = nimcode_config_dir
            .join("skills")
            .join("omc-learned")
            .join("learned");
        ...
        let original_config_home = std::env::var("NIMCODE_CONFIG_HOME").ok();
        let original_codex_home = std::env::var("CODEX_HOME").ok();
        let original_nimcode_config_dir = std::env::var("NIMCODE_CONFIG_DIR").ok();
        std::env::set_var("HOME", &home);
        std::env::remove_var("NIMCODE_CONFIG_HOME");
        std::env::remove_var("CODEX_HOME");
        ...
        match original_nimcode_config_dir {
            Some(value) => std::env::set_var("NIMCODE_CONFIG_DIR", value),
            None => std::env::remove_var("NIMCODE_CONFIG_DIR"),
        }
        fs::remove_dir_all(root).expect("temp tree should clean up");
    }

    #[test]
    fn skill_loads_direct_skill_and_legacy_command_from_nimcode_config_dir() {
        let _guard = env_guard();
        let root = temp_path("nimcode-config-direct-skill");
        let home = root.join("home");
        let nimcode_config_dir = root.join("nimcode-config");
        let skill_dir = nimcode_config_dir.join("skills").join("statusline");
        let command_dir = nimcode_config_dir.join("commands");
        fs::create_dir_all(&skill_dir).expect("direct skill dir should exist");
        fs::create_dir_all(&command_dir).expect("command dir should exist");
        fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: statusline\ndescription: NimCode config skill\n---\n# statusline\n",
        )
        .expect("direct skill file should exist");
        fs::write(
            command_dir.join("doctor-check.md"),
            "---\nname: doctor-check\ndescription: NimCode config command\n---\n# doctor-check\n",
        )
        ...
        assert_eq!(direct_skill_output["description"], "NimCode config skill");
    ...
    fn skill_loads_project_local_legacy_command_markdown() {
        ...
        let workspace = root.join("workspace");
        let nested = workspace.join("nested");
        let command_dir = workspace.join(".nimcode-compat").join("commands");
        ...
        assert!(output["path"]
            .as_str()
            .expect("path")
            .ends_with(".nimcode-compat/commands/team.md"));
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("NIMCODE_AGENT_STORE", &dir);
        ...
        std::env::remove_var("NIMCODE_AGENT_STORE");
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".nimcode")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".nimcode")).expect("cwd dir");
        std::fs::write(
            home.join(".nimcode").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".nimcode")).expect("cwd dir");
        std::fs::write(
            cwd.join(".nimcode").join("settings.local.json"),
        ...
        let local_settings = std::fs::read_to_string(cwd.join(".nimcode").join("settings.local.json"))
            .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".nimcode").join("tool-state").join("plan-mode.json"))
...
"#,
    ...
    fn run_task_packet_creates_packet_backed_task() {
        ...
        assert_eq!(output["task_packet"]["repo"], "nimcode-parity");
        assert_eq!(
