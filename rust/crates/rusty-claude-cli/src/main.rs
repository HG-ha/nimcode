...
/// After the NIM-only refactor the default model is GLM 5.1 served via
...
enum ModelSource {
    ...
    Env,
    /// `model` key in `.nimcode.json` / `.nimcode/settings.json` (when neither
    /// flag nor env set it).
    Config,
...
fn max_tokens_for_model(model: &str) -> u32 {
    // Defer to the api crate, which keeps the per-model output budget for the
    // NIM-served models in `model_token_limit`. Hard-coding 64K here was a
    // legacy Anthropic-era default and would push the request over GLM 5.1's
    // 128K context window once the system prompt + tool registry loaded
    // (Input ~91K + reserved Output 64K = 155K > 131K).
...
const OFFICIAL_REPO_URL: &str = "https://github.com/HG-ha/nimcode";
const OFFICIAL_REPO_SLUG: &str = "HG-ha/nimcode";
const INSTALL_COMMAND: &str = "cargo install --path crates/rusty-claude-cli";
...
// Two-tier resolution:
//
// 1. A process-wide preference (`LOCALE_PREF`) set by the `--lang` CLI flag,
//    `/lang` slash command, or `NIMCODE_LANG` env var. `Forced(_)` always wins.
// 2. When the preference is `Auto`, we probe the system: `LC_ALL` /
//    `LC_MESSAGES` / `LANG` / `LANGUAGE` env vars, then on Windows the
//    `GetUserDefaultUILanguage` Win32 API. Anything we can't classify falls
//    back to English so the CLI never silently switches modes for a user
//    whose locale we don't understand.
...
impl Locale {
    ...
    /// Parse a locale identifier from a user-supplied or env-supplied string.
    /// Accepts BCP-47 (`zh-CN`), POSIX (`zh_CN.UTF-8`), and bare codes (`zh`).
    /// Returns `None` for anything we don't have translations for.
    #[must_use]
    pub fn from_code(s: &str) -> Option<Self> {
        ...
        let head = normalized
            ...
            .unwrap_or("");
        match head {
            "zh" | "zh-cn" | "zh-hans" | "zh-hans-cn" | "zh-tw" | "zh-hant" | "zh-hk"
            | "zh-sg" | "chinese" | "chs" | "cht" => Some(Self::Zh),
            "en" | "en-us" | "en-gb" | "en-au" | "en-ca" | "english" | "c" | "posix" => {
                Some(Self::En)
            }
            other if other.starts_with("zh-") || other == "zh" => Some(Self::Zh),
            other if other.starts_with("en-") || other == "en" => Some(Self::En),
            _ => None,
        }
    }
}
...
static LOCALE_PREF: Mutex<LocalePref> = Mutex::new(LocalePref::Auto);

#[must_use]
pub fn current_locale() -> Locale {
    let pref = LOCALE_PREF
        .lock()
        .map(|guard| *guard)
        .unwrap_or(LocalePref::Auto);
    match pref {
        LocalePref::Forced(locale) => locale,
        LocalePref::Auto => detect_system_locale().unwrap_or(Locale::En),
    }
}

#[must_use]
pub fn current_locale_pref() -> LocalePref {
    LOCALE_PREF
        .lock()
        .map(|guard| *guard)
        .unwrap_or(LocalePref::Auto)
...
pub fn set_locale_preference(pref: LocalePref) {
    if let Ok(mut guard) = LOCALE_PREF.lock() {
        *guard = pref;
    }
}
...
fn detect_system_locale() -> Option<Locale> {
    if let Ok(nimcode_lang) = env::var("NIMCODE_LANG") {
        if !nimcode_lang.is_empty() && !nimcode_lang.eq_ignore_ascii_case("auto") {
            if let Some(loc) = Locale::from_code(&nimcode_lang) {
                return Some(loc);
            }
        }
    }
    for var in ["LC_ALL", "LC_MESSAGES", "LANG", "LANGUAGE"] {
        if let Ok(value) = env::var(var) {
            if !value.is_empty() {
                if let Some(loc) = Locale::from_code(&value) {
                    return Some(loc);
                }
            }
        }
    ...
    if let Some(name) = sys_locale::get_locale() {
        if let Some(loc) = Locale::from_code(&name) {
            return Some(loc);
        }
    }
    // Last-resort Windows fallback: ask PowerShell for the UI culture name.
    // sys_locale may fail in sandboxed / headless environments.
    #[cfg(windows)]
    {
        if let Some(loc) = windows_powershell_locale() {
            return Some(loc);
        }
    }
    None
...
/// Fallback locale detection on Windows: spawn a tiny PowerShell snippet that
/// prints the `CurrentUICulture.Name` (e.g. `zh-CN`). This is slower (~50 ms)
/// but works even when `sys_locale` fails to probe the Win32 API.
#[cfg(windows)]
#[must_use]
fn windows_powershell_locale() -> Option<Locale> {
    ...
    Locale::from_code(name.trim())
}

/// Pick between two strings based on the active locale. Used as a tiny
/// shorthand instead of pulling in a real i18n library — every translated
/// string lives next to its English counterpart at the call site.
#[must_use]
fn t<'a>(en: &'a str, zh: &'a str) -> &'a str {
...
/// Owned-string variant of [`t`] for `format!` outputs.
#[must_use]
...
/// Apply a forced locale override coming from `--lang` / `NIMCODE_LANG`. `auto`
/// resets the preference back to system-driven detection.
fn apply_lang_override(value: &str) -> Result<(), String> {
    let trimmed = value.trim();
    if trimmed.is_empty() || trimmed.eq_ignore_ascii_case("auto") {
        set_locale_preference(LocalePref::Auto);
        return Ok(());
    }
    match Locale::from_code(trimmed) {
        Some(locale) => {
            set_locale_preference(LocalePref::Forced(locale));
            Ok(())
        }
        None => Err(format!(
            "unsupported language '{trimmed}'. Use one of: zh, en, auto"
        )),
    }
}
...
const CREDENTIALS_ENV_KEY: &str = "NVIDIA_NIM_API_KEY";

fn credentials_file_path() -> PathBuf {
    if let Ok(home) = env::var("NIMCODE_CONFIG_HOME") {
        return PathBuf::from(home).join("credentials");
    }
    #[cfg(windows)]
    {
        if let Some(appdata) = env::var_os("APPDATA") {
            return PathBuf::from(appdata).join("nimcode").join("credentials");
        }
    }
    if let Some(home) = env::var_os("HOME")
        .or_else(|| env::var_os("USERPROFILE"))
    {
        return PathBuf::from(home).join(".nimcode").join("credentials");
    }
    PathBuf::from(".nimcode").join("credentials")
}
...
fn load_saved_api_key() -> Option<String> {
    ...
    for line in content.lines() {
        ...
        if let Some(value) = trimmed.strip_prefix("NVIDIA_NIM_API_KEY=") {
...
fn save_api_key(key: &str) -> io::Result<()> {
    ...
    fs::write(&path, format!("NVIDIA_NIM_API_KEY={key}\n"))?;
    #[cfg(unix)]
    {
...
fn prompt_for_api_key() -> Option<String> {
    ...
    let prompt_msg = match locale {
        Locale::Zh => "\n未找到 NVIDIA NIM API Key。\n从 https://build.nvidia.com/ 获取你的 API Key。\n\n请输入 API Key (nvapi-...): ",
...
fn ensure_api_key() {
    if env::var(CREDENTIALS_ENV_KEY)
        .ok()
        .filter(|v| !v.is_empty())
        .is_some()
    {
    ...
    if let Some(key) = load_saved_api_key() {
        env::set_var(CREDENTIALS_ENV_KEY, &key);
        return;
    }
    ...
    if let Some(key) = prompt_for_api_key() {
        ...
        match save_api_key(&key) {
            Ok(()) => {
                let path = credentials_file_path();
                let saved_msg = match current_locale() {
                    Locale::Zh => format!("✓ API Key 已保存到 {}\n", path.display()),
                    Locale::En => format!("✓ API Key saved to {}\n", path.display()),
                };
                eprint!("{saved_msg}");
            }
            Err(err) => {
                let warn_msg = match current_locale() {
                    Locale::Zh => format!("⚠ 保存 API Key 失败: {err}（本次会话仍可使用）\n"),
                    Locale::En => format!("⚠ Failed to save API Key: {err} (still usable this session)\n"),
                };
                eprint!("{warn_msg}");
            }
        }
...
fn main() {
    ...
    if let Err(error) = run() {
        ...
        if json_output {
            // #77: classify error by prefix so downstream tools can route without
            ...
            let kind = classify_error_kind(&message);
            let display_message = localize_error_message(&message);
            let help_suffix = t(
                "Run `nimcode --help` for usage.",
                "运行 `nimcode --help` 查看用法。",
            );
            if display_message.contains("`nimcode --help`") {
...
fn localize_error_message(message: &str) -> String {
    if current_locale() != Locale::Zh {
        return message.to_string();
    }
    if message.contains("missing") && message.contains("credentials") {
        let cred_path = credentials_file_path();
        return format!(
            "缺少 NVIDIA NIM 凭据。请通过以下任一方式设置：\n\
             \n  1. 直接运行 nimcode，首次启动时会提示输入并自动保存\n\
             \n  2. 设置环境变量 NVIDIA_NIM_API_KEY\n\
             \n  3. 在当前目录创建 .env 文件，内容为 NVIDIA_NIM_API_KEY=nvapi-...\n\
             \n  4. 手动写入 {}", cred_path.display()
        );
    }
    if message.contains("Context window blocked") {
        return message
            .replace("Context window blocked", "上下文窗口已满")
            .replace("Recovery", "恢复方法")
            .replace("Compact", "压缩历史")
            .replace("Fresh session", "新建会话")
            .replace("Reduce scope", "减少输入内容")
            .replace("Retry", "重试");
    }
    message.to_string()
}
...
enum CliAction {
    ...
    // #146: `nimcode config` and `nimcode diff` are pure-local read-only
...
enum LocalHelpTopic {
    ...
    // `nimcode <subcommand> --help` has one consistent contract.
    Init,
    State,
    Export,
    Version,
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
                    ...
                    ) =>
            {
                ...
                // the arg to the API (e.g. `nimcode prompt --help`) should show
            ...
            "--lang" => {
                ...
                apply_lang_override(value)?;
                index += 2;
            }
            flag if flag.starts_with("--lang=") => {
                apply_lang_override(&flag[7..])?;
                index += 1;
            }
            "-p" => {
                // NimCode compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // NimCode compat: --print makes output non-interactive
    ...
    match rest[0].as_str() {
        ...
        // top-level parser arm produced CliAction::Plugins. That made `nimcode
        // plugins` (and `nimcode plugins --help`, `nimcode plugins list`, ...)
        ...
        "plugins" => {
            ...
            if tail.len() > 2 {
                return Err(format!(
                    "unexpected extra arguments after `nimcode plugins {}`: {}",
                    tail[..2].join(" "),
                    tail[2..].join(" ")
                ));
            }
        ...
        // `.nimcode.json` + `.nimcode/settings.json` from disk, no network, no
        // state mutation). Previously callers had to spin up a session with
        // `nimcode --resume SESSION.jsonl /config` to see their own config,
        ...
        "config" => {
            ...
            if tail.len() > 1 {
                return Err(format!(
                    "unexpected extra arguments after `nimcode config {}`: {}",
                    tail[0],
                    tail[1..].join(" ")
                ));
            }
        ...
        "diff" => {
            if rest.len() > 1 {
                return Err(format!(
                    "unexpected extra arguments after `nimcode diff`: {}",
                    rest[1..].join(" ")
                ));
            }
        ...
        other => {
            if rest.len() == 1 && looks_like_subcommand_typo(other) {
                if let Some(suggestions) = suggest_similar_subcommand(other) {
                    ...
                    message.push_str(
                        "\nRun `nimcode --help` for the full list. If you meant to send a prompt literally, use `nimcode prompt <text>`.",
                    );
                    return Err(message);
                }
            }
            ...
            // `nimcode ""`, `nimcode "   "`, and `nimcode "" ""` silently route to
            ...
            if joined.trim().is_empty() {
                return Err(
                    "empty prompt: provide a subcommand (run `nimcode --help`) or a non-empty prompt string"
                        .to_string(),
                );
            }
            Ok(CliAction::Prompt {
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`nimcode {command_name}` is a slash command. Use `nimcode --resume SESSION.jsonl /{command_name}` or start `nimcode` and run `/{command_name}`."
        )
    } else {
        format!(
            "`nimcode {command_name}` is a slash command. Start `nimcode` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn removed_auth_surface_error(command_name: &str) -> String {
    format!(
        "`nimcode {command_name}` has been removed. Set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN instead."
    )
}
...
fn parse_acp_args(args: &[String], output_format: CliOutputFormat) -> Result<CliAction, String> {
    match args {
        ...
        _ => Err(String::from(
            "unsupported ACP invocation. Use `nimcode acp`, `nimcode acp serve`, `nimcode --acp`, or `nimcode -acp`.",
        )),
    }
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `nimcode` and run it there, or use `nimcode --resume SESSION.jsonl {command_name}` / `nimcode --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `nimcode --help` for CLI usage, or start `nimcode` and use /help.");
    message
}

fn format_unknown_slash_command(name: &str) -> String {
    let mut message = t_owned(
        format!("Unknown slash command: /{name}"),
        format!("未知的斜杠命令：/{name}"),
    );
    let suggest_label = t("Did you mean", "你是否想输入");
    if let Some(suggestions) = render_suggestion_line(suggest_label, &suggest_slash_commands(name))
    {
        message.push('\n');
        message.push_str(&suggestions);
    }
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `nimcode` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
fn validate_model_syntax(model: &str) -> Result<(), String> {
    ...
    // After the NIM-only refactor every model id is shaped as
    // `vendor/model-id` (e.g. `deepseek-ai/deepseek-v4-pro`, `moonshotai/kimi-k2.5`,
    // `qwen/qwen3-coder-480b-a35b-instruct`) and is forwarded to
    // `integrate.api.nvidia.com/v1` verbatim. We also accept an optional
    // `nvidia_nim/` namespace prefix for parity with `free-claude-code`
    // (e.g. `nvidia_nim/deepseek-ai/deepseek-v4-pro`).
    let stripped = trimmed
        .strip_prefix("nvidia_nim/")
        .or_else(|| trimmed.strip_prefix("nim/"))
        .unwrap_or(trimmed);
...
fn default_permission_mode() -> PermissionMode {
    env::var("NIMCODE_PERMISSION_MODE")
        .ok()
        .as_deref()
...
fn format_connected_line(model: &str) -> String {
    ...
    t_owned(
        ...
        format!("已连接：{model}（提供方 {provider}）"),
    )
}

/// Render one row of the startup banner with the dim-label / value layout.
/// Pads the label to a fixed visual column width so values line up regardless
/// of whether the label is ASCII or full-width CJK. Width math treats
/// non-ASCII chars as 2 cells (good enough for our zh/en label set).
#[must_use]
fn banner_row(label: &str, value: &str) -> String {
    const TARGET_WIDTH: usize = 16;
    let visual_width: usize = label
        .chars()
        .map(|c| if c.is_ascii() { 1 } else { 2 })
        .sum();
    let padding = " ".repeat(TARGET_WIDTH.saturating_sub(visual_width).max(2));
    format!("  \x1b[2m{label}\x1b[0m{padding}{value}")
}
...
/// Starts a minimal Model Context Protocol server that exposes nimcode's
...
/// Read `.nimcode/worker-state.json` from the current working directory and print it.
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let state_path = cwd.join(".nimcode").join("worker-state.json");
    if !state_path.exists() {
        ...
        // no `nimcode worker` subcommand), so users/CI had no discoverable path
        ...
        //     Run:   nimcode               # start the REPL (writes state on first turn)
        //     Or:    nimcode prompt <text> # run one non-interactive turn
        //     Then rerun: nimcode state [--output-format json]
        return Err(format!(
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN; `nimcode login` is removed"
                .to_string(),
        ])
...
fn check_install_source_health() -> DiagnosticCheck {
    DiagnosticCheck::new(
        "Install source",
        DiagnosticLevel::Ok,
        format!("source: {OFFICIAL_REPO_SLUG}"),
    )
    .with_details(vec![
        format!("Repo              {OFFICIAL_REPO_URL}"),
        format!("Build from source {INSTALL_COMMAND}"),
    ])
    .with_data(Map::from_iter([
...
const DUMP_MANIFESTS_OVERRIDE_HINT: &str =
    "Hint: set CLAUDE_CODE_UPSTREAM=/path/to/upstream or pass `nimcode dump-manifests --manifests-dir /path/to/upstream`.";
...
fn print_bootstrap_plan(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let phases = runtime::BootstrapPlan::nimcode_default()
        .phases()
        .iter()
...
struct StatusContext {
    ...
    /// #143: when `.nimcode.json` (or another loaded config file) fails to parse,
    ...
    /// `status: "degraded"` so users can distinguish "status ran but config
...
Usage
  ...
  List available models with /model list (or /models)
  Force-refresh the catalog with /model refresh
  Switch models with /model <name|substring>
    e.g. `/model glm5` -> z-ai/glm5
...
用法
  查看当前模型：/model
  列出可用模型：/model list（或 /models）
  强制刷新模型列表：/model refresh
  切换模型：/model <名称或子串>
    例如 `/model glm5` -> z-ai/glm5
         `/model kimi` -> moonshotai/kimi-k2.5"
        ),
    )
}
...
#[must_use]
fn format_lang_report() -> String {
    let active = current_locale();
    let pref = current_locale_pref();
    let pref_label = match pref {
        LocalePref::Auto => t("auto (system-detected)", "自动（跟随系统）"),
        LocalePref::Forced(_) => t("forced (set via --lang or /lang)", "已固定（通过 --lang 或 /lang 设置）"),
    };
    t_owned(
        format!(
            "Language
  Active           {active_label} ({active_code})
  Preference       {pref_label}

Usage
  Inspect with /lang
  Switch with /lang zh   /lang en   /lang auto",
            active_label = active.label(),
            active_code = active.code(),
        ),
        format!(
            "语言
  当前生效          {active_label}（{active_code}）
  偏好设置          {pref_label}

用法
  查看当前设置：/lang
  切换语言：/lang zh   /lang en   /lang auto",
            active_label = active.label(),
            active_code = active.code(),
        ),
    )
...
#[must_use]
fn format_lang_switch_report(previous: Locale, next: Locale) -> String {
    t_owned(
        format!(
            "Language updated
  Previous         {prev_label} ({prev_code})
  Current          {next_label} ({next_code})
  Note             The change applies to the next LLM turn (system prompt updated).",
        ...
        format!(
            "已切换交互语言
  原语言           {prev_label}（{prev_code}）
  新语言           {next_label}（{next_code}）
  说明             下一轮 LLM 调用将使用新语言（system prompt 已更新）。",
            prev_label = previous.label(),
            prev_code = previous.code(),
            next_label = next.label(),
            next_code = next.code(),
...
/// Cached snapshot of the live NIM model catalog (`GET /v1/models`). Populated
/// lazily on the first `/model …` invocation and reused across REPL turns so
/// completions / fuzzy lookup stay fast. Wrapped in a `Mutex<Option<…>>` (not
/// `OnceLock`) so `/model refresh` can clear it on demand.
static NIM_MODEL_CATALOG: Mutex<Option<Result<Vec<String>, String>>> = Mutex::new(None);

fn fetch_nim_model_catalog_blocking() -> Result<Vec<String>, String> {
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to start tokio runtime for /v1/models: {err}"))?;
    runtime
        .block_on(api::fetch_nim_model_ids())
        .map_err(|err| format!("failed to fetch NIM model catalog: {err}"))
}

fn ensure_nim_model_catalog() -> Result<Vec<String>, String> {
    {
        let guard = NIM_MODEL_CATALOG
            .lock()
            .map_err(|err| format!("model catalog mutex poisoned: {err}"))?;
        if let Some(cached) = guard.as_ref() {
            return cached.clone();
        }
    }
    let fetched = fetch_nim_model_catalog_blocking();
    if let Ok(mut guard) = NIM_MODEL_CATALOG.lock() {
        *guard = Some(fetched.clone());
    }
    fetched
}

fn refresh_nim_model_catalog() -> Result<Vec<String>, String> {
    if let Ok(mut guard) = NIM_MODEL_CATALOG.lock() {
        *guard = None;
    }
    ensure_nim_model_catalog()
}

#[must_use]
fn cached_nim_model_catalog() -> Vec<String> {
    NIM_MODEL_CATALOG
        .lock()
        .ok()
...
#[must_use]
fn match_model_in_catalog(input: &str, catalog: &[String]) -> CatalogMatch {
    ...
    if catalog
        .iter()
        .any(|m| m.eq_ignore_ascii_case(&resolved_lower))
    {
        let canonical = catalog
            .iter()
            .find(|m| m.eq_ignore_ascii_case(&resolved_lower))
            .cloned()
            .unwrap_or(resolved);
        return CatalogMatch::Exact(canonical);
    }
    ...
    if candidates.is_empty() {
        let needle_resolved = resolved_lower.clone();
        candidates = catalog
            .iter()
            .filter(|m| m.to_ascii_lowercase().contains(&needle_resolved))
            .cloned()
            .collect();
    }
    ...
    match candidates.len() {
        0 => CatalogMatch::None,
        1 => CatalogMatch::Unique(candidates.into_iter().next().expect("one match")),
        _ => CatalogMatch::Multiple(candidates),
    }
}

#[must_use]
fn format_model_catalog(catalog: &[String], current: &str) -> String {
    if catalog.is_empty() {
        return t(
            "No models cached. Run `/model refresh` (or set NVIDIA_NIM_API_KEY first).",
            "暂无已缓存的模型。请运行 `/model refresh`（或先设置 NVIDIA_NIM_API_KEY）。",
        )
        .to_string();
    }
    let resolved_current = resolve_model_alias_with_config(current);
    let mut out = String::from(t(
        "Available NVIDIA NIM models\n",
        "可用的 NVIDIA NIM 模型\n",
    ));
    for id in catalog {
        let marker = if id.eq_ignore_ascii_case(&resolved_current) {
            "*"
        } else {
            " "
        };
    ...
    out.push_str(t(
        "\n  * = current model\nUsage: /model <name-or-substring>   (e.g. `/model glm5`, `/model kimi-thinking`)",
        "\n  * = 当前模型\n用法：/model <名称或子串>（例如 `/model glm5`、`/model kimi-thinking`）",
    ));
    out
}

#[must_use]
fn format_catalog_fetch_error(err: &str, current: &str) -> String {
    t_owned(
        format!(
            "Could not load model catalog from NIM: {err}\n\
             The catalog is fetched from `<NVIDIA_NIM_BASE_URL>/models` and requires `NVIDIA_NIM_API_KEY`.\n\
             Falling back to manual entry — current model is still `{current}`."
        ),
        format!(
            "无法从 NIM 加载模型列表：{err}\n\
             模型列表通过 `<NVIDIA_NIM_BASE_URL>/models` 获取，需要设置 `NVIDIA_NIM_API_KEY`。\n\
             回退为手动输入 —— 当前模型仍为 `{current}`。"
        ),
    )
}
...
fn format_permissions_report(mode: &str) -> String {
    let modes = [
        ("read-only", t("Read/search tools only", "仅读取/搜索工具"), mode == "read-only"),
        (
            "workspace-write",
            t("Edit files inside the workspace", "在工作区内编辑文件"),
            mode == "workspace-write",
        ),
        (
            "danger-full-access",
            t("Unrestricted tool access", "不受限制的工具访问"),
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .nimcode/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
  Tip              use /session list to inspect saved sessions"
        ),
        format!(
            "恢复会话
  用法              /resume <会话路径|会话ID|{LATEST_SESSION_REFERENCE}>
  自动保存          .nimcode/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
  提示              使用 /session list 查看已保存的会话"
        ),
    )
}
    ...
    match command {
        ...
        SlashCommand::Clear { confirm } => {
            ...
            Ok(ResumeCommandOutcome {
                session: cleared,
                message: Some(format!(
                    "Session cleared\n  Mode             resumed session reset\n  Previous session {previous_session_id}\n  Backup           {}\n  Resume previous  nimcode --resume {}\n  New session      {new_session_id}\n  Session file     {}",
                    backup_path.display(),
                    backup_path.display(),
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `nimcode` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: nimcode is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && nimcode",
            cwd.display()
        );
        ...
        let message = format!(
            "nimcode is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && nimcode",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        let logo = "\x1b[38;5;45m\
╔═══════════════════════════╗\n\
║      N i m C o d e        ║\n\
╚═══════════════════════════╝\x1b[0m";
        let rows = [
            banner_row(t("Model", "模型"), &self.model),
            banner_row(t("Permissions", "权限"), self.permission_mode.as_str()),
            banner_row(t("Branch", "分支"), git_branch),
            banner_row(t("Workspace", "工作区"), &workspace),
            banner_row(t("Directory", "目录"), &cwd),
            banner_row(t("Session", "会话"), &self.session.id),
            banner_row(t("Auto-save", "自动保存"), &session_path),
        ]
        .join("\n");
        let hint = t(
            "Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
            "输入 \x1b[1m/help\x1b[0m 查看命令 · \x1b[1m/status\x1b[0m 查看实时上下文 · \x1b[2m/resume latest\x1b[0m 跳回最新会话 · \x1b[1m/diff\x1b[0m 然后 \x1b[1m/commit\x1b[0m 提交 · \x1b[2mTab\x1b[0m 自动补全 · \x1b[2mShift+Enter\x1b[0m 换行",
        );
        let lang_hint = t(
            "Run \x1b[1m/lang zh\x1b[0m to interact in 简体中文",
            "输入 \x1b[1m/lang en\x1b[0m 切回英文交互",
        );
        format!("{logo}\n\n{rows}\n\n  {hint}\n  {lang_hint}")
    }
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::Lang { mode } => self.set_lang(mode)?,
    ...
    fn set_model(&mut self, model: Option<String>) -> Result<bool, Box<dyn std::error::Error>> {
        let Some(arg) = model else {
            // Bare `/model` shows the current model, the session counters,
            // and (best-effort) the live NIM catalog so users can pick the
            // next id without having to remember the full vendor/model slug.
            println!(
                "{}",
                format_model_report(
                    &self.model,
            ...
            match ensure_nim_model_catalog() {
                Ok(catalog) => {
                    println!();
                    println!("{}", format_model_catalog(&catalog, &self.model));
                }
                Err(err) => {
                    println!();
                    println!("{}", format_catalog_fetch_error(&err, &self.model));
                }
            }
            return Ok(false);
        };
        ...
        match trimmed.to_ascii_lowercase().as_str() {
            "list" | "ls" => {
                match ensure_nim_model_catalog() {
                    Ok(catalog) => println!("{}", format_model_catalog(&catalog, &self.model)),
                    Err(err) => println!("{}", format_catalog_fetch_error(&err, &self.model)),
                }
                return Ok(false);
            }
            "refresh" | "reload" => {
                match refresh_nim_model_catalog() {
                    Ok(catalog) => {
                        println!("Refreshed NIM model catalog ({} ids).", catalog.len());
                        println!();
                        println!("{}", format_model_catalog(&catalog, &self.model));
                    }
                    Err(err) => println!("{}", format_catalog_fetch_error(&err, &self.model)),
        ...
        // Try the live catalog first so `/model glm5` switches to
        // `z-ai/glm5` and `/model kimi-think` resolves to
        // `moonshotai/kimi-k2-thinking` without the user having to type the
        // full vendor-prefixed slug.
        let catalog = ensure_nim_model_catalog().unwrap_or_default();
        let model = match match_model_in_catalog(trimmed, &catalog) {
            CatalogMatch::Exact(canonical) | CatalogMatch::Unique(canonical) => canonical,
            CatalogMatch::Multiple(candidates) => {
                println!(
                    "{}",
                    t_owned(
                        format!("Multiple NIM models match `{trimmed}`. Pick one and rerun:"),
                        format!("多个 NIM 模型匹配 `{trimmed}`，请选择一个后重试："),
                    )
                );
            ...
            CatalogMatch::None => {
                // Fall back to the existing alias table + verbatim id path so
                // brand-new ids the catalog hasn't yet indexed still work.
                let resolved = resolve_model_alias_with_config(trimmed);
                if !catalog.is_empty() && !catalog.iter().any(|m| m.eq_ignore_ascii_case(&resolved))
                {
                    println!(
                        "{}",
                        t_owned(
                            format!("No NIM model matches `{trimmed}`. Run `/model list` to see the live catalog or `/model refresh` to reload it."),
                            format!("没有匹配 `{trimmed}` 的 NIM 模型。运行 `/model list` 查看可用模型，或 `/model refresh` 刷新列表。"),
                        )
                    );
                    return Ok(false);
                }
    ...
    fn set_lang(&mut self, mode: Option<String>) -> Result<bool, Box<dyn std::error::Error>> {
        let Some(raw) = mode else {
            println!("{}", format_lang_report());
            return Ok(false);
        };

        let trimmed = raw.trim();
        let new_pref = if trimmed.is_empty() || trimmed.eq_ignore_ascii_case("auto") {
            LocalePref::Auto
        } else {
            match Locale::from_code(trimmed) {
                Some(locale) => LocalePref::Forced(locale),
                None => {
                    println!(
                        "{}",
                        t_owned(
                            format!(
                                "Unsupported language `{trimmed}`. Use `/lang zh`, `/lang en`, or `/lang auto`."
                            ),
                            format!(
                                "不支持的语言 `{trimmed}`，可用值：`/lang zh`、`/lang en` 或 `/lang auto`。"
                            ),
                        )
                    );
                    return Ok(false);
        ...
        let previous_pref = current_locale_pref();
        let previous_locale = current_locale();
        set_locale_preference(new_pref);
        let next_locale = current_locale();

        if previous_pref == new_pref && previous_locale == next_locale {
            println!("{}", format_lang_report());
            return Ok(false);
        }

        // The interaction language is folded into the system prompt the
        // runtime sends to the LLM, so re-build the runtime to reflect the
        // change on the next turn. Existing session messages are preserved.
        let session = self.runtime.session().clone();
        ...
        println!("{}", format_lang_switch_report(previous_locale, next_locale));
        Ok(true)
    }
    ...
    fn print_mcp(
        ...
        // `nimcode mcp serve` starts a stdio MCP server exposing nimcode's built-in
...
fn render_repl_help() -> String {
    let en_help = [
        ...
        "  Auto-save            .nimcode/sessions/<session-id>.jsonl".to_string(),
    ...
    if current_locale() == Locale::Zh {
        localize_help_text(&en_help)
    } else {
        en_help
    }
}

fn localize_help_text(help: &str) -> String {
    help
        // REPL section header & shortcuts
        .replace("REPL", "交互终端")
        .replace("Quit the REPL", "退出交互终端")
        .replace("Navigate prompt history", "浏览输入历史")
        .replace("Reverse-search prompt history", "反向搜索输入历史")
        .replace("Complete commands, modes, and recent sessions", "补全命令、模式和最近会话")
        .replace("Clear input (or exit on empty prompt)", "清空输入（空行时退出）")
        .replace("Insert a newline", "插入换行")
        .replace("Resume latest", "恢复最新会话")
        .replace("Browse sessions", "浏览会话")
        .replace("Show prompt history", "显示输入历史")
        // Slash commands section
        .replace("Slash commands", "斜杠命令")
        .replace("Start here", "从这里开始")
        .replace("[resume]          also works with --resume SESSION.jsonl", "[resume]          亦可通过 --resume SESSION.jsonl 使用")
        // Category headers
        .replace("\nSession\n", "\n会话\n")
        .replace("\nTools\n", "\n工具\n")
        .replace("\nConfig\n", "\n配置\n")
        .replace("\nDebug\n", "\n调试\n")
        // Command summaries
        .replace("Show available slash commands", "显示可用的斜杠命令")
        .replace("Show current session status", "显示当前会话状态")
        .replace("Compact local session history", "压缩本地会话历史")
        .replace("Start a fresh local session", "开始新的本地会话")
        .replace("Show cumulative token usage for this session", "显示本会话累计 token 用量")
        .replace("Load a saved session into the REPL", "加载已保存的会话")
        .replace("Show CLI version and build information", "显示 CLI 版本和构建信息")
        .replace("List, switch, fork, or delete managed local sessions", "列出、切换、分叉或删除本地管理的会话")
        .replace("Show workspace and session statistics", "显示工作区和会话统计")
        .replace("Show conversation history summary", "显示对话历史摘要")
        .replace("Show token count for the current conversation", "显示当前对话的 token 数量")
        .replace("Show prompt cache statistics", "显示提示缓存统计")
        .replace("Show or switch the interaction language (zh/en/auto)", "显示或切换交互语言（zh/en/auto）")
        .replace("Inspect configured MCP servers", "查看已配置的 MCP 服务器")
        .replace("Create a starter NIMCODE.md for this repo", "为此仓库创建初始 NIMCODE.md")
        .replace("Show git diff for current workspace changes", "显示当前工作区的 git diff")
        .replace("Inspect the codebase for likely bugs", "检查代码库中的潜在 bug")
        .replace("Generate a commit message and create a git commit", "生成提交信息并创建 git commit")
        .replace("Draft or create a pull request from the conversation", "根据对话起草或创建 pull request")
        .replace("Draft or create a GitHub issue from the conversation", "根据对话起草或创建 GitHub issue")
        .replace("Run a deep planning prompt with multi-step reasoning", "运行深度规划提示（多步推理）")
        .replace("Jump to a file or symbol by searching the workspace", "通过搜索工作区跳转到文件或符号")
        .replace("Export the current conversation to a file", "将当前对话导出到文件")
        .replace("Manage NimCode plugins", "管理 NimCode 插件")
        .replace("List configured agents", "列出已配置的 agents")
        .replace("List, install, or invoke available skills", "列出、安装或调用可用 skills")
        .replace("Approve a pending tool execution", "批准待执行的工具操作")
        .replace("Deny a pending tool execution", "拒绝待执行的工具操作")
        .replace("Show or switch the active model", "显示或切换当前模型")
        .replace("Show or switch the active permission mode", "显示或切换当前权限模式")
        .replace("Inspect NimCode config files or merged sections", "查看配置文件或合并后的配置段")
        .replace("Inspect loaded NimCode instruction memory files", "查看已加载的指令记忆文件")
        .replace("List available model providers", "列出可用的模型提供方")
        .replace("Show sandbox isolation status", "显示沙盒隔离状态")
        .replace("Replay the last tool call with debug details", "重放最后一次工具调用（含调试详情）")
        .replace("Diagnose setup issues and environment health", "诊断环境配置问题")
}
...
fn status_json_value(
    ...
    // #143: top-level `status` marker so users can distinguish
    ...
    json!({
        ...
        "workspace": {
            ...
            "session_id": context.session_path.as_ref().and_then(|path| {
                ...
                // .nimcode/sessions/. Extract the stem (drop the .jsonl extension).
...
fn status_context(
    ...
    // `nimcode doctor` already does this; `nimcode status` now matches that contract
    ...
    let (loaded_config_files, sandbox_status, config_load_error) = match loader.load() {
        ...
        Err(err) => (
            0,
            // Fall back to defaults for sandbox resolution so users still see
...
fn format_status_report(
    ...
    if let Some(err) = context.config_load_error.as_deref() {
        blocks.push(format!(
            "Config load error\n  Status           fail\n  Summary          runtime config failed to load; reporting partial status\n  Details          {err}\n  Hint             `nimcode doctor` classifies config parse errors; fix the listed field and rerun"
        ));
    }
  ...
  累计合计          {}",
        ...
        {
            let ws_cwd = context.cwd.display().to_string();
            let ws_root = context
                .project_root
                .as_ref()
                .map_or_else(|| "unknown".to_string(), |path| path.display().to_string());
            let ws_branch = context.git_branch.as_deref().unwrap_or("unknown");
            let ws_state = context.git_summary.headline();
            let ws_changed = context.git_summary.changed_files;
            let ws_staged = context.git_summary.staged_files;
            let ws_unstaged = context.git_summary.unstaged_files;
            let ws_untracked = context.git_summary.untracked_files;
            let ws_session = context.session_path.as_ref().map_or_else(
                || "live-repl".to_string(),
                |path| path.display().to_string(),
            );
            let ws_loaded = context.loaded_config_files;
            let ws_discovered = context.discovered_config_files;
            let ws_memory = context.memory_file_count;
            t_owned(
                format!(
                    "Workspace
  Cwd              {ws_cwd}
  Project root     {ws_root}
  Git branch       {ws_branch}
  Git state        {ws_state}
  Changed files    {ws_changed}
  Staged           {ws_staged}
  Unstaged         {ws_unstaged}
  Untracked        {ws_untracked}
  Session          {ws_session}
  Config files     loaded {ws_loaded}/{ws_discovered}
  Memory files     {ws_memory}
  ...
  未跟踪            {ws_untracked}
  会话              {ws_session}
  配置文件          已加载 {ws_loaded}/{ws_discovered}
  Memory 文件       {ws_memory}
...
fn format_sandbox_report(status: &runtime::SandboxStatus) -> String {
    let label = |en, zh| t(en, zh);
    format!(
        "{title}
  {l_enabled:<18}{enabled}
  {l_active:<18}{active}
  {l_supported:<18}{supported}
  {l_container:<18}{in_container}
  {l_req_ns:<18}{req_ns}
  {l_act_ns:<18}{act_ns}
  {l_req_net:<18}{req_net}
  {l_act_net:<18}{act_net}
  {l_fs_mode:<18}{fs_mode}
  {l_fs_active:<18}{fs_active}
  {l_mounts:<18}{mounts}
  {l_markers:<18}{markers}
  {l_fallback:<18}{fallback}",
        title = label("Sandbox", "沙盒"),
        l_enabled = label("Enabled", "已启用"),
        l_active = label("Active", "活跃"),
        l_supported = label("Supported", "受支持"),
        l_container = label("In container", "容器内"),
        l_req_ns = label("Requested ns", "已请求命名空间"),
        l_act_ns = label("Active ns", "活跃命名空间"),
        l_req_net = label("Requested net", "已请求网络"),
        l_act_net = label("Active net", "活跃网络"),
        l_fs_mode = label("Filesystem mode", "文件系统模式"),
        l_fs_active = label("Filesystem active", "文件系统活跃"),
        l_mounts = label("Allowed mounts", "允许的挂载"),
        l_markers = label("Markers", "标记"),
        l_fallback = label("Fallback reason", "回退原因"),
        enabled = status.enabled,
        active = status.active,
        supported = status.supported,
        in_container = status.in_container,
        req_ns = status.requested.namespace_restrictions,
        act_ns = status.namespace_active,
        req_net = status.requested.network_isolation,
        act_net = status.network_active,
        fs_mode = status.filesystem_mode.as_str(),
        fs_active = status.filesystem_active,
        mounts = if status.allowed_mounts.is_empty() {
            t("<none>", "<无>").to_string()
        } else {
            status.allowed_mounts.join(", ")
        },
        markers = if status.container_markers.is_empty() {
            t("<none>", "<无>").to_string()
        } else {
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            nimcode status [--output-format <format>]
  Purpose          show the local workspace snapshot without entering the REPL
  Output           model, permissions, git state, config files, and sandbox status
  Formats          text (default), json
  Related          /status · nimcode --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            nimcode sandbox [--output-format <format>]
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Formats          text (default), json
  Related          /sandbox · nimcode status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            nimcode doctor [--output-format <format>]
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Formats          text (default), json
  Related          /doctor · nimcode --resume latest /doctor"
            .to_string(),
        LocalHelpTopic::Acp => "ACP / Zed
  Usage            nimcode acp [serve] [--output-format <format>]
  Aliases          nimcode --acp · nimcode -acp
  ...
  Formats          text (default), json
  Related          ROADMAP #64a (discoverability) · ROADMAP #76 (real ACP support) · nimcode --help"
            .to_string(),
        LocalHelpTopic::Init => "Init
  Usage            nimcode init [--output-format <format>]
  Purpose          create .nimcode/, .nimcode.json, .gitignore, and NIMCODE.md in the current project
  Output           list of created vs. skipped files (idempotent: safe to re-run)
  Formats          text (default), json
  Related          nimcode status · nimcode doctor"
            .to_string(),
        LocalHelpTopic::State => "State
  Usage            nimcode state [--output-format <format>]
  Purpose          read .nimcode/worker-state.json written by the interactive REPL or a one-shot prompt
  Output           worker id, model, permissions, session reference (text or json)
  Formats          text (default), json
  Produces state   `nimcode` (interactive REPL) or `nimcode prompt <text>` (one non-interactive turn)
  Observes state   `nimcode state` reads; clawhip/CI may poll this file without HTTP
  Exit codes       0 if state file exists and parses; 1 with actionable hint otherwise
  Related          nimcode status · ROADMAP #139 (this worker-concept contract)"
            .to_string(),
        LocalHelpTopic::Export => "Export
  Usage            nimcode export [--session <id|latest>] [--output <path>] [--output-format <format>]
  Purpose          serialize a managed session to JSON for review, transfer, or archival
  Defaults         --session latest (most recent managed session in .nimcode/sessions/)
  Formats          text (default), json
  Related          /session list · nimcode --resume latest"
            .to_string(),
        LocalHelpTopic::Version => "Version
  Usage            nimcode version [--output-format <format>]
  Aliases          nimcode --version · nimcode -V
  Purpose          print the nimcode CLI version and build metadata
  Formats          text (default), json
  Related          nimcode doctor (full build/auth/config diagnostic)"
            .to_string(),
        LocalHelpTopic::SystemPrompt => "System Prompt
  Usage            nimcode system-prompt [--cwd <path>] [--date YYYY-MM-DD] [--output-format <format>]
  Purpose          render the resolved system prompt that `nimcode` would send for the given cwd + date
  Options          --cwd overrides the workspace dir · --date injects a deterministic date stamp
  Formats          text (default), json
  Related          nimcode doctor · nimcode dump-manifests"
            .to_string(),
        LocalHelpTopic::DumpManifests => "Dump Manifests
  Usage            nimcode dump-manifests [--manifests-dir <path>] [--output-format <format>]
  ...
  Formats          text (default), json
  Related          nimcode skills · nimcode agents · nimcode doctor"
            .to_string(),
        LocalHelpTopic::BootstrapPlan => "Bootstrap Plan
  Usage            nimcode bootstrap-plan [--output-format <format>]
  ...
  Formats          text (default), json
  Related          nimcode doctor · nimcode status"
            .to_string(),
    }
}
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let message = "ACP/Zed editor integration is not implemented in nimcode yet. `nimcode acp serve` is only a discoverability alias today; it does not launch a daemon or Zed-specific protocol endpoint. Use the normal terminal surfaces for now and track ROADMAP #76 for real ACP support.";
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           discoverability only\n  Launch           `nimcode acp serve` / `nimcode --acp` / `nimcode -acp` report status only; no editor daemon is available yet\n  Today            use `nimcode prompt`, the REPL, or `nimcode doctor` for local verification\n  Tracking         ROADMAP #76\n  Message          {message}"
            );
        }
        CliOutputFormat::Json => {
            println!(
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No NIMCODE instruction files discovered in the current directory ancestry."
                .to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
fn init_nimcode_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
...
/// string so users can detect per-artifact state without substring matching.
...
fn build_system_prompt() -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    if let Some(directive) = locale_system_prompt_section() {
        sections.push(directive);
    }
    Ok(sections)
}

/// Render the language directive injected into the LLM's system prompt.
/// Returns `None` for English (LLMs default to mirroring the user's input
/// language already), so we only override behaviour when the user has
/// explicitly opted into a non-English mode.
#[must_use]
fn locale_system_prompt_section() -> Option<String> {
    match current_locale() {
        Locale::Zh => Some(
            "# Interaction language\n\
             - The user has selected 简体中文 (zh) as the interaction language.\n\
             - 默认使用简体中文回答用户的问题，包括解释、状态汇报和工具调用前后的说明。\n\
             - Code, identifiers, file paths, command names, log lines, and error \
               messages must remain in their original language (typically English).\n\
             - 如果用户主动用其他语言提问，跟随用户当前使用的语言作答。"
                .to_string(),
        ),
        Locale::En => None,
    }
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // After the NIM-only refactor every model resolves through
        // `ApiProviderClient::Nim`, which wraps an `OpenAiCompatClient`
        // configured for `integrate.api.nvidia.com/v1`. The single call
        // below reads `NVIDIA_NIM_API_KEY` (and optional
        // `NVIDIA_NIM_BASE_URL`) on its own — there is no per-provider
        // dispatch to perform. Prompt-cache support was Anthropic-only
        // and has been removed; the `session_id` parameter is preserved
        // for downstream telemetry threading.
        //
        // We canonicalize the model id here (alias expansion + stripping
        // any leading `nvidia_nim/` namespace) so the value forwarded in
        // the request body matches what NIM publishes under
        // `GET /v1/models`. Without this, e.g. `nvidia_nim/deepseek-ai/deepseek-v4-pro`
        // would reach the upstream verbatim and trigger a plaintext
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   nimcode --resume {session_id} /compact"
    ));
...
fn slash_command_completion_candidates_with_sessions(
    ...
    // Fold in the cached NIM model catalog so `Tab` after `/model …` /
    // `/models …` cycles through real, live ids. Only entries already
    // populated via a previous `/model` invocation are surfaced — we don't
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "nimcode v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  nimcode [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  nimcode [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  nimcode [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  nimcode --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  nimcode help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  nimcode version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  nimcode status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  nimcode sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  nimcode doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  nimcode acp [serve]")?;
    writeln!(
        out,
        "      Show ACP/Zed editor integration status (currently unsupported; aliases: --acp, -acp)"
    )?;
    writeln!(out, "      Source: {OFFICIAL_REPO_SLUG}")?;
    writeln!(out, "  nimcode dump-manifests [--manifests-dir PATH]")?;
    writeln!(out, "  nimcode bootstrap-plan")?;
    writeln!(out, "  nimcode agents")?;
    writeln!(out, "  nimcode mcp")?;
    writeln!(out, "  nimcode skills")?;
    writeln!(out, "  nimcode system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(out, "  nimcode init")?;
    writeln!(
        out,
        "  nimcode export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .nimcode/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  nimcode --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  nimcode --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  nimcode --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  nimcode --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  nimcode --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  nimcode --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  nimcode agents")?;
    writeln!(out, "  nimcode mcp show my-server")?;
    writeln!(out, "  nimcode /skills")?;
    writeln!(out, "  nimcode doctor")?;
    writeln!(out, "  source: {OFFICIAL_REPO_URL}")?;
    writeln!(out, "  nimcode init")?;
    writeln!(out, "  nimcode export")?;
    writeln!(out, "  nimcode export conversation.md")?;
    Ok(())
}
...
mod tests {
    use super::{
        ...
        write_mcp_server_fixture, current_locale_pref, set_locale_preference, CliAction,
        CliOutputFormat, CliToolExecutor, GitWorkspaceSummary, InternalPromptProgressEvent,
        InternalPromptProgressState, LiveCli, Locale, LocalHelpTopic, LocalePref,
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   nimcode --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("nimcode-cli-{nanos}-{unique}"))
    }
    ...
    fn write_plugin_fixture(root: &Path, name: &str, include_hooks: bool, include_lifecycle: bool) {
        fs::create_dir_all(root.join(".nimcode-plugin")).expect("manifest dir");
        if include_hooks {
        ...
        fs::write(
            root.join(".nimcode-plugin").join("plugin.json"),
            format!(
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".nimcode")).expect("project config dir should exist");
        ...
        let original_config_home = std::env::var("NIMCODE_CONFIG_HOME").ok();
        let original_permission_mode = std::env::var("NIMCODE_PERMISSION_MODE").ok();
        ...
        match original_config_home {
            ...
            None => std::env::remove_var("NIMCODE_CONFIG_HOME"),
        }
        match original_permission_mode {
            Some(value) => std::env::set_var("NIMCODE_PERMISSION_MODE", value),
            None => std::env::remove_var("NIMCODE_PERMISSION_MODE"),
        }
    ...
    fn env_permission_mode_overrides_project_config_default() {
        ...
        let original_permission_mode = std::env::var("NIMCODE_PERMISSION_MODE").ok();
        std::env::set_var("NIMCODE_CONFIG_HOME", &config_home);
        std::env::set_var("NIMCODE_PERMISSION_MODE", "read-only");
    ...
    fn removed_login_and_logout_subcommands_error_helpfully() {
        ...
        match parse_args(&[
            "--model=nvidia_nim/deepseek-ai/deepseek-v4-pro".to_string(),
            "status".to_string(),
        ])
    ...
    fn status_degrades_gracefully_on_malformed_mcp_config_143() {
        // #143: previously `nimcode status` hard-failed on any config parse error,
        // taking down the entire health surface for one malformed MCP entry.
        // `nimcode doctor` already degrades gracefully; this test locks `status`
...
"#,
        ...
        .expect("write malformed .nimcode.json");
    ...
    fn state_error_surfaces_actionable_worker_commands_139() {
        // #139: the error for missing `.nimcode/worker-state.json` must name
        // the concrete commands that produce worker state, otherwise users
        ...
        assert!(
            message.contains("nimcode prompt"),
            "error should name `nimcode prompt <text>` as a producer: {message}"
        );
        assert!(
        ...
        assert!(
            message.contains("nimcode state"),
        ...
        // so users can discover the contract without hitting the error first.
        ...
        assert!(
            state_help.contains("nimcode prompt"),
            "state help must name `nimcode prompt <text>` as a producer: {state_help}"
        );
    }
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("nimcode --resume SESSION.jsonl /status"));
    }
    ...
    fn punctuation_bearing_single_token_still_dispatches_to_prompt() {
        ...
        // doesn't pick up a stale .nimcode/settings.json from other tests that
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("nimcode --help"));
    }
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .nimcode/sessions/<session-id>.jsonl"));
    ...
    fn startup_banner_mentions_workflow_completions() {
        let _guard = env_lock();
        let previous_locale = current_locale_pref();
        set_locale_preference(LocalePref::Forced(Locale::En));
        // Inject a dummy NIM key so LiveCli can construct without contacting
        // integrate.api.nvidia.com. The banner test never makes real requests.
        std::env::set_var("NVIDIA_NIM_API_KEY", "test-dummy-key-for-banner-test");
        let root = temp_dir();
        ...
        set_locale_preference(previous_locale);
    }

    #[test]
    fn format_connected_line_routes_all_models_through_nvidia_nim() {
        // After the NIM-only refactor every model — regardless of name — is
        // dispatched to the NVIDIA NIM provider.
        let _guard = env_lock();
        let previous = current_locale_pref();
        set_locale_preference(LocalePref::Forced(Locale::En));
        for model in [
            "deepseek-ai/deepseek-v4-pro",
            ...
            assert_eq!(line, format!("Connected: {model} via nvidia_nim"));
        }
        set_locale_preference(previous);
    }

    #[test]
    fn resolve_repl_model_returns_user_supplied_model_unchanged_when_explicit() {
        let user_model = "deepseek-ai/deepseek-v4-pro".to_string();

        let resolved = resolve_repl_model(user_model);

        assert_eq!(resolved, "deepseek-ai/deepseek-v4-pro");
    }

    #[test]
    fn resolve_repl_model_falls_back_to_model_env_when_default() {
        let _guard = env_lock();
        let root = temp_dir();
        fs::create_dir_all(&root).expect("root dir");
        ...
        std::env::remove_var("ANTHROPIC_MODEL");
        std::env::remove_var("NIMCODE_CONFIG_HOME");
        fs::remove_dir_all(root).expect("cleanup temp dir");
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("nimcode help"));
        assert!(help.contains("nimcode version"));
        assert!(help.contains("nimcode status"));
        assert!(help.contains("nimcode sandbox"));
        assert!(help.contains("nimcode init"));
        assert!(help.contains("nimcode acp [serve]"));
        assert!(help.contains("nimcode agents"));
        assert!(help.contains("nimcode mcp"));
        assert!(help.contains("nimcode skills"));
        assert!(help.contains("nimcode /skills"));
        assert!(help.contains("HG-ha/nimcode"));
        assert!(!help.contains("nimcode login"));
        assert!(!help.contains("nimcode logout"));
    }
...
UU conflicted.rs",
    ...
    fn render_diff_report_shows_clean_tree_for_committed_repo() {
        ...
        git(&["config", "user.name", "NimCode Tests"], &root);
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("nimcode --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("nimcode --resume latest"));
        assert!(help.contains("nimcode --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".nimcode/sessions/legacy.json");
    ...
    fn load_session_reference_rejects_workspace_mismatch() {
        ...
        let session_path = workspace_a.join(".nimcode/sessions/legacy-cross.jsonl");
        std::fs::create_dir_all(
            session_path
                .parent()
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".nimcode/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("nimcode-cli-{label}-{nanos}"))
    }
    ...
    fn init_template_mentions_detected_rust_workspace() {
        ...
        let rendered = crate::init::render_init_nimcode_md(&workspace_root);
        assert!(rendered.contains("# NIMCODE.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/nimcode-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p nimcode-cli"}"#)
                .contains("cargo test -p nimcode-cli")
        );
        assert_eq!(
    ...
    // The fixture writes a Python script and invokes it as `python3`; on
    // Windows `python3` is rarely on PATH, so we gate this to Unix.
    #[cfg(unix)]
    #[test]
    #[allow(clippy::too_many_lines)]
    ...
    // The lifecycle fixture writes `#!/bin/sh` scripts and depends on POSIX
    // executable permissions to invoke them, so we gate this test to Unix.
    #[cfg(unix)]
    #[test]
    fn build_runtime_runs_plugin_lifecycle_init_and_shutdown() {
        // Serialize access to process-wide env vars so parallel tests that
        // set/remove NVIDIA_NIM_API_KEY do not race with this test.
        let _guard = env_lock();
        let config_home = temp_dir();
...
mod dump_manifests_tests {
    ...
    fn dump_manifests_shows_helpful_error_when_manifests_missing() {
        let root = std::env::temp_dir().join(format!(
            "nimcode_test_missing_manifests_{}",
            std::process::id()
        ));
    ...
    fn dump_manifests_uses_explicit_manifest_dir() {
        let root = std::env::temp_dir().join(format!(
            "nimcode_test_explicit_manifest_dir_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
