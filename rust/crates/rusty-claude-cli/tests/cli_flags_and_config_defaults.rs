...
fn status_command_applies_model_and_permission_mode_flags() {
    ...
    // when
    let output = Command::new(env!("CARGO_BIN_EXE_nimcode"))
        ...
        .output()
        .expect("nimcode should launch");

    // then
    assert_success(&output);
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf8");
    assert!(stdout.contains("Status"));
    // `sonnet` is a CLI alias that, after the NIM-only refactor, resolves to
    // an NVIDIA NIM-served model id rather than an Anthropic Claude name.
    assert!(stdout.contains("Model            deepseek-ai/deepseek-v4-pro") || stdout.contains("模型"));
...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_nimcode"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("nimcode should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_nimcode"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("nimcode should launch");
...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    let config_home = temp_dir.join("home").join(".nimcode");
    fs::create_dir_all(temp_dir.join(".nimcode")).expect("project config dir should exist");
    ...
    fs::write(temp_dir.join(".nimcode.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".nimcode").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = command_in(&temp_dir)
        .env("NIMCODE_CONFIG_HOME", &config_home)
        .args([
            "--resume",
            session_path.to_str().expect("utf8 path"),
            "/config",
    ...
    assert!(stdout.contains(temp_dir.join(".nimcode.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".nimcode")
...
fn doctor_command_runs_as_a_local_shell_entrypoint() {
    ...
    let output = command_in(&temp_dir)
        ...
        .env("NVIDIA_NIM_BASE_URL", "http://127.0.0.1:9")
        .arg("doctor")
        .output()
        .expect("nimcode doctor should launch");
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            nimcode doctor"));
    ...
    assert!(status_stdout.contains("Usage            nimcode status"));
...
fn command_in(cwd: &Path) -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_nimcode"));
