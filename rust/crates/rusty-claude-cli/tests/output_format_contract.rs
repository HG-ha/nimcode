...
fn agents_command_emits_structured_agent_entries_when_requested() {
    ...
    assert_eq!(parsed["agents"][0]["source"]["id"], "project_nimcode");
    ...
    assert_eq!(parsed["agents"][2]["shadowed_by"]["id"], "project_nimcode");
}
...
fn dump_manifests_and_init_emit_json_when_requested() {
    ...
    assert!(workspace.join("NIMCODE.md").exists());
}
...
fn doctor_and_resume_status_emit_json_when_requested() {
    ...
    assert_eq!(
        ...
        "https://github.com/HG-ha/nimcode"
    );
...
fn resumed_version_and_init_emit_structured_json_when_requested() {
    ...
    assert_eq!(init["kind"], "init");
    assert!(root.join("NIMCODE.md").exists());
}
...
fn assert_json_command_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Value {
    let output = run_nimcode(current_dir, args, envs);
    assert!(
...
fn run_nimcode(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
...
fn write_upstream_fixture(root: &Path) -> PathBuf {
    let upstream = root.join("nimcode");
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "nimcode-output-format-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
