...
const MANIFEST_RELATIVE_PATH: &str = ".nimcode-plugin/plugin.json";
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("NIMCODE_PLUGIN_ID", &self.plugin_id)
            .env("NIMCODE_PLUGIN_NAME", &self.plugin_name)
            .env("NIMCODE_TOOL_NAME", &self.definition.name)
            .env("NIMCODE_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("NIMCODE_PLUGIN_ROOT", root.display().to_string());
        }
...
fn load_manifest_from_path(
    ...
    let compatibility_errors = detect_nimcode_manifest_contract_gaps(&raw_json);
...
fn detect_nimcode_manifest_contract_gaps(
    raw_manifest: &Value,
    ...
    for (field, detail) in [
        (
            "skills",
            "plugin manifest field `skills` uses the Claude Code plugin contract; `nimcode` does not load plugin-managed skills and instead discovers skills from local roots such as `.nimcode/skills`, `.omc/skills`, `.agents/skills`, `~/.omc/skills`, and `~/.nimcode/skills/omc-learned`.",
        ),
        (
            "mcpServers",
            "plugin manifest field `mcpServers` uses the Claude Code plugin contract; `nimcode` does not import MCP servers from plugin manifests.",
        ),
        (
            "agents",
            "plugin manifest field `agents` uses the Claude Code plugin contract; `nimcode` does not load plugin-managed agent markdown catalogs from plugin manifests.",
        ),
    ] {
    ...
    {
        errors.push(PluginManifestValidationError::UnsupportedManifestContract {
            detail: "plugin manifest field `commands` uses Claude Code-style directory globs; `nimcode` slash dispatch is still built-in and does not load plugin slash command markdown files.".to_string(),
        });
    }
    ...
    if let Some(hooks) = root.get("hooks").and_then(Value::as_object) {
        for hook_name in hooks.keys() {
            if !matches!(
                ...
                errors.push(PluginManifestValidationError::UnsupportedManifestContract {
                    detail: format!(
                        "plugin hook `{hook_name}` uses the Claude Code lifecycle contract; `nimcode` plugins currently support only PreToolUse, PostToolUse, and PostToolUseFailure."
                    ),
                });
            }
        }
...
/// Guards against concurrent modification of `NIMCODE_CONFIG_HOME`.
#[cfg(test)]
...
mod tests {
  ...
  "commands": [
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        ...
        write_file(
            &script_path,
            "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$NIMCODE_PLUGIN_ID\" \"$NIMCODE_TOOL_NAME\" \"$INPUT\"\n",
        );
        #[cfg(unix)]
        {
  ...
  "commands": [
    ...
    fn load_plugin_from_directory_rejects_claude_code_manifest_contracts_with_guidance() {
        let root = temp_dir("manifest-nimcode-contract");
        write_file(
            root.join(MANIFEST_FILE_NAME).as_path(),
            r#"{
  ...
  "mcpServers": "./.mcp.json"
        ...
        assert!(rendered.contains("field `skills` uses the Claude Code plugin contract; `nimcode`"));
        assert!(rendered.contains("field `mcpServers` uses the Claude Code plugin contract; `nimcode`"));
        assert!(rendered.contains("field `agents` uses the Claude Code plugin contract; `nimcode`"));
        assert!(rendered.contains("field `commands` uses Claude Code-style directory globs; `nimcode`"));
        assert!(rendered.contains("hook `SessionStart` uses the Claude Code lifecycle contract; `nimcode`"));
  ...
  "description": "Packaged manifest in install root"
    ...
    /// Regression test for ROADMAP #41: verify that `NIMCODE_CONFIG_HOME` isolation prevents
    /// host `~/.nimcode/plugins/` from bleeding into test runs.
    #[test]
    fn nimcode_config_home_isolation_prevents_host_plugin_leakage() {
        let _guard = env_guard();

        // Create a temp directory to act as our isolated NIMCODE_CONFIG_HOME
        let config_home = temp_dir("isolated-home");
        let bundled_root = temp_dir("isolated-bundled");

        // Set NIMCODE_CONFIG_HOME to our temp directory
  ...
  "description": "Test fixture plugin in isolated config home"
        ...
        // Create PluginManager with isolated bundled_root - it should use the temp config_home, not host ~/.nimcode/
        let mut config = PluginManagerConfig::new(&config_home);
        config.bundled_root = Some(bundled_root.clone());
        let manager = PluginManager::new(config);
        ...
        assert_eq!(
            installed.len(),
            1,
            "should only see the test fixture plugin, not host ~/.nimcode/plugins/"
        );
        assert_eq!(
