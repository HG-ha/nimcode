...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    if let Some(explicit) = std::env::var_os("NIMCODE_UPSTREAM") {
        candidates.push(PathBuf::from(explicit));
    }

    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("nimcode"));
        candidates.push(ancestor.join("nimcode-upstream"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("nimcode"));
    candidates.push(primary_repo_root.join("vendor").join("nimcode"));
