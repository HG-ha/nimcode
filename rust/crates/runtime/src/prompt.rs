...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("NIMCODE.md"),
            dir.join("NIMCODE.local.md"),
            dir.join(".nimcode").join("NIMCODE.md"),
            dir.join(".nimcode").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "NimCode instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# NimCode instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No NimCode settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".nimcode")).expect("nested nimcode dir");
        fs::write(root.join("NIMCODE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("NIMCODE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".nimcode")).expect("apps nimcode dir");
        fs::write(root.join("apps").join("NIMCODE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".nimcode").join("instructions.md"),
            "apps dot nimcode instructions",
        )
        .expect("write apps dot nimcode instructions");
        fs::write(nested.join(".nimcode").join("NIMCODE.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".nimcode").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("NIMCODE.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("NIMCODE.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.nimcode/NIMCODE.md")),
            "NIMCODE.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("NIMCODE.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? NIMCODE.md"));
    ...
    #[test]
    fn load_system_prompt_reads_nimcode_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".nimcode")).expect("nimcode dir");
        fs::write(root.join("NIMCODE.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".nimcode").join("settings.json"),
        ...
        let original_nimcode_home = std::env::var("NIMCODE_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("NIMCODE_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_nimcode_home {
            std::env::set_var("NIMCODE_CONFIG_HOME", value);
        } else {
    ...
    #[test]
    fn renders_nimcode_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".nimcode")).expect("nimcode dir");
        fs::write(root.join("NIMCODE.md"), "Project rules").expect("write NIMCODE.md");
        fs::write(
            root.join(".nimcode").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
    ...
    #[test]
    fn discovers_dot_nimcode_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".nimcode")).expect("nested nimcode dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".nimcode/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/NIMCODE.md"),
