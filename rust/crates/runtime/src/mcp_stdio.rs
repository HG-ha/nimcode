...
// These tests rely on POSIX file permissions (`set_mode`) and `/bin/sh` shell
// scripts, so they are gated to Unix targets. On Windows the module is simply
// skipped — equivalent coverage is provided through higher-level integration
