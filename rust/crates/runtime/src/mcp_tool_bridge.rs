...
// Gated to Unix targets because the embedded fixture scripts rely on `chmod`
// (`PermissionsExt::set_mode`) and `python3` being on the PATH. The CLI MCP
// bridge itself is portable; only this fixture-driven harness is Unix-only.
#[cfg(all(test, unix))]
mod tests {
